...
if [ "$NODE_TYPE" == "Master" ]; then
    ...
    aws ssm put-parameter --name "/k3s/cluster/master-ip" --value "$(curl -H "X-aws-ec2-metadata-token: $TOKEN" -s "http://169.254.169.254/latest/meta-data/local-ipv4")" --type String --overwrite
else
    KS_TOKEN=$(aws ssm get-parameter --name "/k3s/cluster/token" --with-decryption --query "Parameter.Value" --output text)
    MASTER_IP=$(aws ssm get-parameter --name "/k3s/cluster/master-ip" --query "Parameter.Value" --output text)
    SERVER_URL="https://$MASTER_IP:6443"
    curl -sfL https://get.k3s.io | K3S_URL=$SERVER_URL K3S_TOKEN=$KS_TOKEN sh -s - server --cluster-init --disable-apiserver --disable-controller-manager --disable-scheduler
fi